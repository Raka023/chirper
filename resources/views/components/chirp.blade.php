        ...
        <div class="flex items-center justify-between">
            <div class="space-x-2">
                <p class="text-sm leading-7 text-gray-400">
                    ...
                    @if ($chirp->updated_at->gt($chirp->created_at->addSeconds(5)))
                        <span> - </span>
                        <span class="italic">Updated {{ $chirp->updated_at->diffForHumans() == '0 seconds ago' ? 'Just now' : $chirp->updated_at->diffForHumans() }}</span>
                    @endif
                </p>
            </div>
            <div class="flex gap-2">
                ...
                <form action="{{ route('chirps.destroy', $chirp->id) }}" method="POST">
                    @csrf
                    @method('DELETE')
                <x-ui.button type="submit" variant="danger" size="sm" onclick="return confirm('Are you sure?')">Delete</x-ui.button>
                </form>
            </div>
        </div>
    </div>
